    ...
    <aside class="signin-aside">
      <div class="signin-aside-top">
        <div class="signin-logo">
          <span class="signin-logo-mark"
            ><img src="/op-icons/logo.png" alt="umu"
          /></span>
      ...
      <div class="signin-aside-body">
        ...
        <p class="signin-welcome-sub">
          Good to see you again. Your Property Passport is right where you left
          it.
        </p>
      ...
      <div class="signin-main-inner">
            ...
            <section class="signin-panel-wrap">
              ...
              <form class="auth-form signin-panel" @submit.prevent="onPrimary">
                ...
                <template v-if="resetStep === 'idle'">
                  ...
                  <button
                    type="submit"
                    class="btn-primary btn-primary--futuristic"
                  ...
                  <div v-if="newPassword" class="password-strength">
                    <div class="strength-bars">
                      <div
                        ...
                        :style="{
                          background:
                            n <= passwordStrength ? strengthBg : '#eef0f6',
                        }"
                      />
                    </div>
              ...
              <div v-if="resetStep === 'idle'" class="signin-divider">
                <span>new to UMU?</span>
              </div>

              <div
                v-if="resetStep === 'idle'"
                class="auth-footer signin-footer"
              >
                New to UmovingU?
                <NuxtLink to="/onboarding/signup">Create an account.</NuxtLink>
              </div>
            </section>
          </div>
        </template>
        ...
        <template v-else-if="resetStep === 'sent'">
          <div class="confirm-state confirm-state--futuristic">
            ...
            <div class="confirm-sub">
              We've sent a password reset code to<br /><strong>{{
                resetEmail
              }}</strong
              >.<br /><br />
              It should arrive in a minute or two. Check your spam folder if you
              can't see it.
            </div>
            <button class="btn-primary" @click="resetStep = 'otp'">
              Enter the code
            </button>
            <button
              type="button"
        ...
        <template v-else-if="resetStep === 'success'">
          <div class="confirm-state confirm-state--futuristic">
            ...
            <button class="btn-primary" @click="resetStep = 'idle'">
              Back to sign in
            </button>
          </div>
        </template>
...
const formSub = computed(() => {
  ...
  if (resetStep.value === "email")
    return "Enter your email and we'll send you a code to set a new password.";
  if (resetStep.value === "otp")
    return "Enter the 6-digit code we just sent you.";
  if (resetStep.value === "newPassword")
    return "At least 8 characters. Mix in a number for extra strength.";
  return "";
});
...
const handleVerifyResetOtp = async () => {
  ...
  try {
    const res: any = await $fetch(
      `${config.public.apiBase}/auth/verify-reset-otp`,
      {
        method: "POST",
...
.signin-aside::after {
  ...
  background: linear-gradient(
    180deg,
    rgba(35, 29, 69, 0) 0%,
    rgba(35, 29, 69, 0.08) 22%,
    rgba(35, 29, 69, 0.08) 78%,
    rgba(35, 29, 69, 0) 100%
  );
}
...
@media (min-width: 881px) {
  ...
  .signin-passport-illus {
    ...
    width: clamp(
      120px,
      calc(((100vh - 80px) / var(--desk-zoom, 1) - 376px) / 1.22),
      300px
    );
...
@media (min-width: 881px) and (max-height: 700px) {
  ...
  .signin-split .signin-aside {
    padding-block: 24px;
  }
  .signin-split .signin-aside-body {
    padding: 24px 0 0;
  }
  .signin-split .signin-welcome {
    font-size: clamp(48px, 4.6vw, 60px);
  }
  .signin-split .signin-welcome-sub {
    margin-top: 8px;
    font-size: 15px;
  }
  .signin-split .signin-aside-foot {
    margin-top: 16px;
  }
  .signin-split .signin-passport-illus {
    width: clamp(
      120px,
      calc((100vh / var(--desk-zoom, 1) - 330px) / 1.22),
      300px
    );
    margin-top: 12px;
  }
...
@media (min-width: 881px) and (max-height: 620px) {
  ...
  .signin-split .signin-form-head {
    margin-bottom: 14px;
  }
  .signin-split .signin-form-title {
    font-size: 25px;
  }
  .signin-split .signin-form-sub {
    margin-top: 4px;
  }
  .signin-split .signin-panel-wrap .form-field {
    margin-bottom: 12px;
  }
  .signin-split .signin-panel-wrap .form-label {
    margin-bottom: 6px;
  }
  .signin-split .signin-panel-wrap .form-input {
    padding-block: 11px;
  }
  .signin-split .signin-panel .btn-primary--futuristic {
    padding-block: 13px;
  }
  .signin-split .signin-panel .btn-text {
    padding-block: 6px;
    margin-top: 4px;
  }
  .signin-split .signin-divider {
    margin: 12px 0 2px;
  }
  .signin-split .logged-out-toast {
    margin-bottom: 14px;
  }
}
...
.signin-panel-wrap .form-input.with-icon {
  padding-left: 42px;
}
.signin-panel-wrap .form-input::placeholder {
  color: #a39fb2;
  font-weight: 500;
}
...
.signin-panel-wrap .form-input-action {
  color: #a39fb2;
}
...
@media (min-width: 881px) {
  .signin-split .signin-welcome.signin-welcome--long {
...
@media (min-width: 1536px) {
  ...
  .signin-aside {
    padding: 40px clamp(48px, 5.5vw, 150px);
  }
  .signin-main {
    padding-inline: clamp(40px, 5vw, 140px);
  }
}
