...
const { convertLandlordToSeller, activatePassport } = usePassportClaim()
// One activate-and-reload per page visit - see loadPassport.
...
async function loadPassport() {
  ...
  try {
    ...
    if (sections.value.length <= 1 && !seedAttempted) {
      seedAttempted = true
      try {
        await activatePassport(passportId.value)
        return await loadPassport()
      } catch (err) {
        console.error('[landlord] activate failed for', passportId.value, err)
      }
    }
