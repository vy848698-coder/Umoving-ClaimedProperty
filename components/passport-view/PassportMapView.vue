...
const mapLayout = {
  rightOffsetX: 165,
  // Numbers, not strings: '-20' + 104 concatenated to '-20104px' and threw the
  // walking lady off the map whenever she stood on a left-hand step.
  leftOffsetX: -20,
  stepGapY: -31,
}
...
const ladyPosition = computed(() => {
  ...
  const isRight = index % 2 === 0
  // `.map-step` is position: relative, so steps stack in flow and each one
  // sits STEP_ADVANCE below the last - not at index * stepGapY.
  const stepTop = index * STEP_ADVANCE
